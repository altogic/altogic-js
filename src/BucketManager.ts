...
export class BucketManager extends APIBase {
   ...
   async empty(): Promise<{ errors: APIError | null }> {
      const { errors } = await this.fetcher.post(`/_api/rest/v1/storage/bucket/empty`, {
         bucket: this.#bucketNameOrId,
      });
   ...
   async delete(): Promise<{ errors: APIError | null }> {
      ...
      const { errors } = await this.fetcher.post(`/_api/rest/v1/storage/bucket/delete`, {
         bucket: this.#bucketNameOrId,
      });
   ...
   async deleteFiles(fileNamesOrIds: string[]): Promise<{ errors: APIError | null }> {
      ...
      const { errors } = await this.fetcher.post(`/_api/rest/v1/storage/bucket/delete-files`, {
         fileNamesOrIds,
         bucket: this.#bucketNameOrId,
      });
