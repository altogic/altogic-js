...
export class CacheManager extends APIBase {
   ...
   async delete(keys: string | string[]): Promise<{ errors: APIError | null }> {
      ...
      const { errors } = await this.fetcher.delete(`/_api/rest/v1/cache`, { keys: keysVal });
      return { errors };
   }
   ...
   async increment(
      ...
      const { data, errors } = await this.fetcher.post('/_api/rest/v1/cache/increment', {
         key,
         increment,
         ttl: ttl ? ttl : undefined,
      });
   ...
   async decrement(
      ...
      const { data, errors } = await this.fetcher.post('/_api/rest/v1/cache/decrement', {
         key,
         decrement,
         ttl: ttl ? ttl : undefined,
      });
   ...
   async listKeys(
      pattern: string,
      next: string
   ): Promise<{ data: object[] | null; next: string | null; errors: APIError | null }> {
      const { data, errors } = await this.fetcher.post(`/_api/rest/v1/cache/list-keys`, {
         pattern,
         next,
      });
