...
export class AuthManager extends APIBase {
  ...
  async signInWithCode(
    ...
    const { data, errors } = await this.fetcher.post(
      `/_api/rest/v1/auth/signin-code?code=${code}&phone=${encodeURIComponent(
        phone
      )}`
    );
   ...
   */
  signInWithProvider(
    provider: "google" | "facebook" | "twitter" | "discord" | "github"
  ): void {
  ...
  async changePhone(
    ...
    checkRequired("currentPassword", currentPassword);
    checkRequired("newPhone", newPhone);
  ...
  async verifyPhone(
    ...
    const { data, errors } = await this.fetcher.post(
      `/_api/rest/v1/auth/verify-phone?code=${code}&phone=${encodeURIComponent(
        phone
      )}`
    );
