...
export class AuthManager extends APIBase {
   ...
   async getAuthGrant(accessToken?: string): Promise<{
      ...
      const { data, errors } = await this.fetcher.get(`/_api/rest/v1/auth/grant?key=${tokenStr}`);
   ...
   async sendMagicLinkEmail(email: string): Promise<{ errors: APIError | null }> {
      ...
      const { errors } = await this.fetcher.post(`/_api/rest/v1/auth/send-magic?email=${email}`);
      return { errors };
   }
   ...
   async sendResetPwdEmail(email: string): Promise<{ errors: APIError | null }> {
      ...
      const { errors } = await this.fetcher.post(`/_api/rest/v1/auth/send-reset?email=${email}`);
      return { errors };
   }
