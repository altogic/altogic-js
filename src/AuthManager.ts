...
export class AuthManager extends APIBase {
   ...
   setSessionCookie(sessionToken: string, req: any, res: any): void {
      setCookie(req, res, 'session_token', sessionToken, 60 * 60 * 24 * 365, 'none', true, true);
   }
   ...
   async signUpWithEmail(
      ...
      const { data, errors } = await this.fetcher.post('/_api/rest/v1/auth/signup-email', {
         email,
         password,
         name,
      });
   ...
   async signUpWithPhone(
      ...
      const { data, errors } = await this.fetcher.post('/_api/rest/v1/auth/signup-phone', {
         phone,
         password,
         name,
      });
   ...
   async signInWithEmail(
      ...
      const { data, errors } = await this.fetcher.post('/_api/rest/v1/auth/signin-email', {
         email,
         password,
      });
   ...
   async signInWithPhone(
      ...
      const { data, errors } = await this.fetcher.post('/_api/rest/v1/auth/signin-phone', {
         phone,
         password,
      });
   ...
   async signOutAll(): Promise<{ errors: APIError | null }> {
      const { errors } = await this.fetcher.post('/_api/rest/v1/auth/signout-all');
   ...
   async signOutAllExceptCurrent(): Promise<{ errors: APIError | null }> {
      const { errors } = await this.fetcher.post('/_api/rest/v1/auth/signout-all-except');
   ...
   async getAllSessions(): Promise<{
      ...
      const { data, errors } = await this.fetcher.get('/_api/rest/v1/auth/sessions');
   ...
   async changePassword(
      ...
      const { errors } = await this.fetcher.post('/_api/rest/v1/auth/change-pwd', {
         newPassword,
         oldPassword,
      });
      return { errors };
   ...
   async resendVerificationEmail(email: string): Promise<{ errors: APIError | null }> {
      const { errors } = await this.fetcher.post(`/_api/rest/v1/auth/resend?email=${email ?? ''}`);
      return { errors };
   }
    ...
    */
   async resendVerificationCode(phone: string): Promise<{ errors: APIError | null }> {
   ...
   async resetPwdWithCode(
      ...
      const { errors } = await this.fetcher.post(
         `/_api/rest/v1/auth/reset-pwd-code?phone=${encodeURIComponent(phone)}&code=${code ?? ''}`,
         {
            newPassword,
         }
      );
   ...
   async changeEmail(
      ...
      const { data, errors } = await this.fetcher.post(`/_api/rest/v1/auth/change-email`, {
         currentPassword,
         newEmail,
      });
   ...
   async changePhone(
      ...
      const { data, errors } = await this.fetcher.post(`/_api/rest/v1/auth/change-phone`, {
         currentPassword,
         newPhone,
      });
